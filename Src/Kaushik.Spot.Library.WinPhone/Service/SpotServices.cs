    ...
    public static class SpotServices
    {
        ...
        public static string ExecuteCommand(string commandText)
        {
            ...
            else
            {
                ...
                try
                {
                    command.Deserialize(SpotServices.cipher.Decrypt(commandText));
                    result = ExecuteCommand(command);
                    resultText = SpotServices.cipher.Encrypt(result.Serialize());
                }
                catch (Exception error)
                {
                    result = new ServiceResult();
